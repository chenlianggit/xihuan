...
class AddressController extends PublicController {
    ...
    public function add_adds(){
        ...
        if ($check_id) {
            echo json_encode(array('status'=>1,'err'=>'该地址已经添加了.','add_arr'=>['addr_id'=>$check_id]));
            exit();
        }
        $province = M('china_city')->where('name='.intval($data['sheng']))->getField('name');
        $city_name = M('china_city')->where('name='.intval($data['city']))->getField('name');
        $quyu_name = M('china_city')->where('name='.intval($data['quyu']))->getField('name');
        $data['address_xq'] = $data['sheng'].' '.$data['city'].' '.$data['quyu'].' '.$data['address'];
     ...
     * 微信获取收货地址
     */
    public function set_wechat_address(){
        /**
         *
        ...
        $telNumber=intval($_REQUEST['telNumber']);
        $provinceName=intval($_REQUEST['provinceName']);
        $cityName=intval($_REQUEST['cityName']);
        $countyName=intval($_REQUEST['countyName']);
        $detailInfo=intval($_REQUEST['detailInfo']);

        //获取省份数据
        $china_city=M("china_city");
        $provicelist = $china_city->where('tid=0')->field('id,name')->select();
        //获取该省份ID
