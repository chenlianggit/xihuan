...
class OrderController extends PublicController {
	...
	public function index(){
		...
		foreach ($order as $n=>$v) {
			...
			foreach($prolist as $k=>$v) {
				...
				$order[$n]['plist'][$k]['photo_x'] = __DATAURL__.$v['photo_x'];
				$order[$n]['plist'][$k]['pid'] = $v['pid'];
				$order[$n]['plist'][$k]['name'] = $v['name'];
				$order[$n]['plist'][$k]['price_yh'] = $v['price'];
				$order[$n]['plist'][$k]['num'] = $v['num'];
				$order[$n]['plist'][$k]['pro_buff'] = $v['pro_buff'];
	...
	public function order_details(){
		...
		//获取物流代码
		$post_info = $post->where('name='."'".$order_info['kuaidi_name']."'")->find();

		echo json_encode(array('status'=>1,'pro'=>$pro,'ord'=>$order_info,'post_code'=>$post_info['code']));
		exit();
	}
	...
	public function add_comment(){
		...
		//先判断是否已经存在评论，如果已经存在，就更新，如果不存在就插入新评价
		$product_dp_info = $product_dp->where('uid='.$uid.' and pid='.$pid.' and order_id='.$order_id)->find();
		if ($product_dp_info){
			$result  = $product_dp->where('id='.$product_dp_info['id'])->save($data);
			if($result){
				echo json_encode(array('status'=>1));
			}else{
			...
			if($result){
				//评价完后用户获得积分
				$product_info = $product->where('id='.$pid)->find();
				$price_jf = $product_info['price_jf'];
				//获取用户数据
				$user_jifen = $user->where('id='.$uid)->find();
				$user_data['jifen'] = intval($user_jifen) + intval($price_jf);
				$user->where('id='.$uid)->save($user_data);
				Log::write('积分：'.$user_data['jifen']);
	...
	public function order_express(){
		//echo 'ddd';$id              = $_POST['pid'];
		$ShipperCode = $_POST['post_code'];
		$LogisticCode = $_POST['kuaidi_num'];
		echo getOrderTracesByJson($ShipperCode,$LogisticCode);
		exit;
	}
