...
class ShoppingController extends PublicController {
	...
	public function add_group(){
		...
		if($res){
			//如果是开团的话
			if($isGroupBuy==1){
				$group_data['pid']=$group_join['pid'];
				$group_data['group_id']=$group_join['id'];
				$group_data['uid']=$uid;
				$group_data['need_count']=$group_join['group_count'];
				$group_data['buy_count']=1;
				$group_data['time']=time();
				$group_data['status']=1;
				$result = M('group_joins_open')->where("1=1")->add($group_data);
			}
