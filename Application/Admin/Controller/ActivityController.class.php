...
class ActivityController extends PrivilegeController{
	...
	public function _initialize(){
		...
		$this->activity_product = D('activity_product');
	}
	...
	public function index(){
		...
		$adv_list = $this->activity->where($condition)->limit($Page->firstRow.','.$Page->listRows)->order('create_at desc')->select();		
		...
		if (!empty($_FILES["file"]["tmp_name"])) {
			...
			if(!is_array($info)) {// 上传错误提示错误信息
				...
				if (intval($_POST['adv_id'])) {
					$check_url = $this->activity->where('id='.intval($_POST['id']))->getField('photo');
					$url = "Data/".$check_url;
	...
	public function add_product() {
		...
		$productlist=M('product')->where($where)->order('addtime desc')->limit(0,999999)->select();
		...
		$select_product_list = M('activity_product')->where(['activity_id'=>$_REQUEST['adv_id']])->select();
		
		$this->assign('productlist',$productlist);
		$this->assign('select_product_list',$select_product_list);
		$this->assign('activity_id',$_REQUEST['adv_id']);
		$this->display();
	}

	public function save_product() {
		foreach( $_REQUEST['my-select'] as $key => $val ) {
			$item = array('activity_id'=>$_REQUEST['activity_id'],'pid'=>$val,'create_at'=>time(),'update_at'=>0);
			$this->activity_product->add($item);
		}
	...
	public function del(){
		...
		$check_info = $this->activity->where('id='.intval($adv_id))->find();
		if (!$check_info) {
		...
		//修改对应的删除状态
		$up = $this->activity->where('id='.intval($adv_id))->delete();
		if ($up) {
