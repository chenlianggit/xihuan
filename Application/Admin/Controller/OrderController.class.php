...
class OrderController extends PrivilegeController{
	...
	public function index(){
	    ...
	    */
		...
		foreach ($order_list as $k => $v) {
			//$order_list[$k]['u_name'] = M('user')->where('id='.intval($v['uid']))->find();
			$order_list[$k]['u_name'] = M('user')->where('id='.intval($v['uid']))->getField('uname');
		}
	...
	*/
	public function show(){
		...
		$post_info = array();
		$post_infos = M('post')->where('1=1')->field('id,name')->select();
		if (intval($order_info['post'])) {
			$post_info = M('post')->where('id='.intval($order_info['post']))->find();
		}

        $this->assign('post_infos',$post_infos);
	...
	*/
	public function sms_up(){
        ...
        //通过kuaidi_name查询出快递的id，然后更新order表的post字段
        $post_info = M('post')->where('name="'.$kuaidi_name.'"')->find();
